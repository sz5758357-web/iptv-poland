...
function render(){
    ...
    .forEach(ch => {
        ...
        // Kliknięcie w kanał: odtwarza wideo i na telefonie przełącza widok na odtwarzacz
        card.onclick = () => {
            play(ch);
            if(window.innerWidth <= 768) {
                // Przełącz zakładkę na odtwarzacz automatycznie
                const playerBtn = document.querySelector('.mob-btn');
                if(playerBtn) switchTab('player', playerBtn);
            }
        };
...
function play(channel){
    ...
    // Przykładowe EPG – możesz podpiąć własne źródło
    epg.innerHTML = `
...
// Obsługa kategorii w menu bocznym
document.querySelectorAll(".category").forEach(btn => {
    btn.onclick = () => {
        ...
        // Na telefonie po wybraniu kategorii przenosi z powrotem do listy kanałów
        if(window.innerWidth <= 768) {
            const channelsNavBtn = document.querySelectorAll('.mob-btn')[1];
            if(channelsNavBtn) switchTab('channels', channelsNavBtn);
        }
    };
});
